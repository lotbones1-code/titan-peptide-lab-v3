...
// the URL bar. This page now:
//   - opens the drawer if there's anything in it (so the user sees their cart)
//   - otherwise shows a clean empty-cart message + link to the catalog
//   - never 404s
export default function CartPage() {
  const { items, hydrated, itemCount, setIsOpen } = useCart();

  useEffect(() => {
    if (hydrated && itemCount > 0) {
      setIsOpen(true);
    }
  }, [hydrated, itemCount, setIsOpen]);
          ...
          {!hydrated ? (
            ...
            <>
              ...
              <p className="max-w-md text-[14px] leading-[1.7] text-[#5c6762]">
                Browse the catalog — nasal sprays from $59, vials and stacks
            ...
            <>
              ...
              <p className="max-w-md text-[14px] leading-[1.7] text-[#5c6762]">
                ...
                Use the drawer on the right to review and check out, or jump
